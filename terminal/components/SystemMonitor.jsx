...
const MonitorGraph = ({ label, data, color, error, redMask, redHistory }) => {
      ...
      <div className="flex-1 w-full min-h-0 -ml-2">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            ...
            <Line type="linear" dataKey="value" stroke={color} strokeWidth={2} dot={false} isAnimationActive={false} strokeLinecap="square" strokeLinejoin="miter" />
            {Array.from({ length: maxHist }, (_, i) => i + 1).map(level => {
              const opacity = (level / (maxHist + 3)) * 0.45;
              return (
                <Line key={`hist-${level}`} type="linear" dataKey={`redHist${level}`} stroke="#f87171" strokeWidth={2} dot={false} isAnimationActive={false} strokeOpacity={opacity} connectNulls={false} strokeLinecap="square" strokeLinejoin="miter" />
              );
            })}
            {Array.from({ length: maxLife }, (_, i) => i + 1).map(level => {
              const opacity = 0.6 + (level / (maxLife + 1)) * 0.4;
              return (
                <Line key={`red-${level}`} type="linear" dataKey={`redMask${level}`} stroke="#f87171" strokeWidth={2} dot={false} isAnimationActive={false} strokeOpacity={opacity} connectNulls={false} strokeLinecap="square" strokeLinejoin="miter" />
              );
            })}
          </LineChart>
        </ResponsiveContainer>
...
const SystemMonitor = ({ isSpookyActive = false }) => {
  const [cpuData, setCpuData] = useState(new Array(20).fill({ value: 0 }));
  const [memData, setMemData] = useState(new Array(20).fill({ value: 0 }));
  const [cpuError, setCpuError] = useState(false);
  const [memError, setMemError] = useState(false);
  const [cpuMask, setCpuMask] = useState(new Array(20).fill(0));
  const [memMask, setMemMask] = useState(new Array(20).fill(0));
  const [cpuHistory, setCpuHistory] = useState(new Array(20).fill(0));
  const [memHistory, setMemHistory] = useState(new Array(20).fill(0));
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCpuData(prev => {
        ...
        if (rand < 0.1) newValue = prevValue;
        else if (rand < 0.6) newValue = Math.floor(Math.random() * 70) + 5;
        else {
      ...
      setCpuMask(prev => {
        ...
        shifted.push(0);
        if (Math.random() < 0.08) {
          const cpuLen = Math.floor(Math.random() * 4) + 2;
          const cpuLife = Math.floor(Math.random() * 3) + 2;
      ...
      setMemData(prev => {
        ...
        const rand = Math.random();
        if (rand < 0.1) newValue = prevValue;
        else if (rand < 0.55) newValue = Math.floor(Math.random() * 50) + 20;
        else {
      ...
      setMemMask(prev => {
        ...
        shifted.push(0);
        if (Math.random() < 0.06) {
          const memLen = Math.floor(Math.random() * 4) + 2;
          const memLife = Math.floor(Math.random() * 3) + 2;
