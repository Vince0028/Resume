...
const LiveChat = ({ onExit }) => {
    ...
    const handleUsernameSubmit = async (e) => {
        e.preventDefault();
        setUsernameError('');
        if (!tempUsername.trim()) { setUsernameError('Code name cannot be empty'); return; }
        if (!tempPassword.trim()) { setUsernameError('Password cannot be empty'); return; }
        if (tempPassword.length < 6) { setUsernameError('Password must be at least 6 characters'); return; }
    ...
    const handleMessageSubmit = async (e) => {
        ...
        setInput('');
        if (content.toLowerCase() === '/exit' || content.toLowerCase() === '/quit') { onExit(); return; }
        const now = new Date().toISOString();
        const tempMsg = { id: `temp-${Date.now()}`, username, content, created_at: now };
        setMessages((L) => [...L, tempMsg]);
        if (authPending) { pendingQueueRef.current.push(content); return; }
        try {
            await fetch('/api/chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        return (
            <div className="w-full max-w-2xl mt-4">
                <div className={`${THEME_COLOR} mb-4`}>
                    Initializing Secure Chat Protocol...<br />Connection established.<br />Identity verification required.
                </div>
                {usernameError && (
                    <div className="text-red-400 mb-2 text-sm animate-pulse">⚠ {usernameError}</div>
                )}
                <form onSubmit={handleUsernameSubmit} className="flex flex-col w-full max-w-2xl space-y-2">
                    <label className="flex flex-col">
                        ...
                        <input ref={usernameRef} type="text" value={tempUsername} onChange={(e) => setTempUsername(e.target.value)} onFocus={() => setActiveField('username')} className={`bg-transparent border-none outline-none ${THEME_COLOR} font-mono focus:ring-0 w-full`} placeholder="choose a code name" />
                    </label>
                    <label className="flex flex-col">
                        <span className={`${THEME_COLOR} mr-2`}>Password:</span>
                        <input ref={null} type="password" value={tempPassword} onChange={(e) => setTempPassword(e.target.value)} onFocus={() => setActiveField('password')} className={`bg-transparent border-none outline-none ${THEME_COLOR} font-mono focus:ring-0 w-full`} placeholder="password (min 6)" />
                    </label>
    ...
    return (
        <div className="w-full h-full flex flex-col mt-4">
            ...
            <form onSubmit={handleMessageSubmit} className="flex items-center border-t border-indigo-500/30 pt-2 space-x-2">
                ...
                <input ref={messageRef} type="text" value={input} onChange={(e) => setInput(e.target.value)} onFocus={() => setActiveField('message')} className={`bg-transparent border-none outline-none ${THEME_COLOR} font-mono focus:ring-0 flex-1`} autoFocus placeholder="Type a message... (/exit to leave)" />
            </form>
        </div>
    );
};
