...
export default function DecryptedText({
    ...
    const [displayText, setDisplayText] = useState(animateOn === 'view' ? '' : text);
            ...
            <span aria-hidden="true">
                {displayText.split('').map((char, index) => {
                    const isRevealedOrDone = revealedIndices.has(index) ||
                        ((!isScrambling || !isHovering) && animateOn !== 'view');

                    if (sequential && revealDirection === 'start' && isScrambling && !isRevealedOrDone && index > revealedIndices.size + 2) {
                        return null;
                    }
