...
const BinaryGlobeNetwork = ({ networkLevel, isVoicePlaying, isIAmPlaying }) => {
    const canvasRef = useRef(null);
    const [points, setPoints] = useState([]);
    const [crises, setCrises] = useState([]);

    const stateRef = useRef({
        y: 0, velocity: 0.012, baseSpeed: 0.012, glitchTimer: 0, jitter: 0, pulse: 0,
        glitchIntensity: 0, infectionActive: false, infectionProgress: 0, rebootTimer: 0
    });
    const frameIdRef = useRef(0);

    useEffect(() => {
        const rawPoints = globeData;
        const generatedPoints = rawPoints.map(p => ({ ...p, char: Math.random() > 0.5 ? '1' : '0' }));
        ...
        for (const p of candidates) {
            ...
            for (const existing of crisisPoints) {
                const dx = p.x - existing.x, dy = p.y - existing.y, dz = p.z - existing.z;
                if (Math.sqrt(dx * dx + dy * dy + dz * dz) < minDistance) { tooClose = true; break; }
            }
            if (!tooClose) crisisPoints.push({ x: p.x, y: p.y, z: p.z, label: crisisLabels[Math.floor(Math.random() * crisisLabels.length)] });
        }
    ...
    useEffect(() => {
        ...
        const render = () => {
            ...
            if (!s.infectionActive) {
                if (Math.random() > 0.9995) { s.infectionActive = true; s.infectionProgress = 0; }
            } else {
                s.infectionProgress += 2 + (s.infectionProgress * 0.03);
                if (s.infectionProgress >= points.length + 200) {
                    if (!s.rebootTimer) { s.rebootTimer = 180; s.glitchTimer = 180; }
                    s.rebootTimer--;
                    if (s.rebootTimer > 0) { s.glitchIntensity = 1.0; s.jitter = (Math.random() - 0.5) * 0.05; }
                    if (s.rebootTimer <= 0) { s.infectionActive = false; s.infectionProgress = 0; s.rebootTimer = 0; s.glitchTimer = 0; }
                }
            }
            if (s.glitchTimer > 0) {
                s.glitchTimer--; s.jitter = (Math.random() - 0.5) * 0.03; s.glitchIntensity = Math.random();
                if (Math.random() > 0.8) s.velocity *= 0.4;
            } else {
                s.jitter = 0; s.glitchIntensity = 0;
                if (Math.random() > 0.988) {
                    s.glitchTimer = Math.floor(Math.random() * 20) + 8;
                    s.velocity = Math.random() > 0.6 ? -s.velocity * (1.5 + Math.random()) : (Math.random() - 0.5) * 0.1;
                }
            }
            const targetSpeed = s.velocity < 0 ? -s.baseSpeed : s.baseSpeed;
            s.velocity += (targetSpeed - s.velocity) * 0.04;
            s.y += s.velocity + s.jitter;
            const cosY = Math.cos(s.y), sinY = Math.sin(s.y), cosX = Math.cos(0.25), sinX = Math.sin(0.25);
            ...
            glow.addColorStop(0, glowColor); glow.addColorStop(1, 'rgba(13, 14, 16, 0)');
            bctx.fillStyle = glow; bctx.fillRect(0, 0, width, height);
            ...
            for (let i = 0; i < points.length; i++) {
                ...
                if (rrz > 0) {
                    const screenX = width / 2 + rx * radius, screenY = height / 2 - ry * radius;
            ...
            for (const c of crises) {
                ...
                if (rrz > 0.2) {
                    ...
                    bctx.strokeStyle = `rgba(239, 68, 68, ${op * 0.8})`; bctx.stroke();
                    bctx.beginPath();
                    bctx.arc(screenX, screenY, 2 * rrz, 0, Math.PI * 2);
                    bctx.fillStyle = `rgba(220, 38, 38, ${rrz})`; bctx.fill();
                    if (Math.random() > 0.1 || s.glitchTimer === 0) {
                        ...
                        bctx.beginPath(); bctx.moveTo(screenX, screenY);
                        bctx.lineTo(screenX + 12 * rrz, screenY - 10 * rrz);
                        bctx.strokeStyle = `rgba(239, 68, 68, ${rrz * 0.5})`; bctx.stroke();
                    }
                }
            }
            ...
            if (s.glitchTimer > 0) {
                ...
                for (let i = 0; i < slices; i++) {
                    const h = Math.random() * (height / 8), y = Math.random() * (height - h);
                    const xOffset = (Math.random() - 0.5) * 60 * s.glitchIntensity;
                    ctx.drawImage(buffer, 0, y, width, h, xOffset, y, width, h);
                    if (Math.random() > 0.7) { ctx.fillStyle = `rgba(99, 102, 241, ${Math.random() * 0.2})`; ctx.fillRect(0, y, width, 1); }
                }
                if (Math.random() > 0.5) {
                ...
                if (Math.random() > 0.92) { ctx.fillStyle = `rgba(255, 255, 255, ${Math.random() * 0.08})`; ctx.fillRect(0, Math.random() * height, width, 20 + Math.random() * 100); }
            } else {
    ...
    if (isIAmPlaying) {
        return (
            <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
                <img src="/Images/iam.png" alt="I AM" className="w-full h-full object-cover opacity-90 filter hue-rotate-15 contrast-125 sepia-[.3]" />
    ...
    return (
        <div className="relative w-full h-full flex items-center justify-center">
            <canvas ref={canvasRef} width={300} height={300} className="w-full h-full object-contain cursor-default select-none filter drop-shadow-[0_0_20px_rgba(99,102,241,0.15)]" />
            {points.length === 0 && (
